...
Surface::Surface( int w, int h, uint32_t* b ) : pixels( b ), width( w ), height( h ) {}

Surface::Surface( int w, int h ) : width( w ), height( h )
{
	pixels = (uint32_t*)MALLOC64( w * h * sizeof( uint32_t ) );
...
void Surface::LoadFromFile( const char* file )
{
	...
	if (!data) return; // load failed
		pixels = (uint32_t*)MALLOC64( width * height * sizeof( uint32_t ) );
		ownBuffer = true; // needs to be deleted in destructor
...
void Surface::Plot( int x, int y, uint32_t c )
{
...
void Surface::Bar( int x1, int y1, int x2, int y2, uint32_t c )
{
	// clipping
	if (x1 < 0) x1 = 0;
...
void Surface::Print( const char* s, int x1, int y1, uint32_t c )
{
	if (!fontInitialized)
	{
...
void Surface::Line( float x1, float y1, float x2, float y2, uint32_t c )
{
