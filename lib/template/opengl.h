...
public:
	...
	GLTexture( uint32_t width, uint32_t height, uint32_t type = DEFAULT );
	~GLTexture();
	// methods
...
public:
	...
	void Bind();
	void SetInputTexture( uint32_t slot, const char* name, GLTexture* texture );
	...
	void SetUInt( const char* name, const uint32_t v );
	void Unbind();
private:
	// data members
	uint32_t vertex = 0;	// vertex shader identifier
	uint32_t pixel = 0;		// fragment shader identifier
	uint32_t ID = 0;		// shader program identifier
};
...
GLuint CreateVBO( const GLfloat* data, const uint32_t size );
void BindVBO( const uint32_t idx, const uint32_t N, const GLuint id );
