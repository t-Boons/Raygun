...
// see http://developer.download.nvidia.com/devzone/devcenter/
// 
// 
// 
// 
// 
// graphics/files/OptimusRenderingPolicies.pdf
...
bool IsKeyDown( const uint32_t key ) { return keystate[key & 511] == 1; }
...
void CharEventCallback( GLFWwindow*, uint32_t ) { /* nothing here yet */ }
...
void main()
{
	...
	((Renderer*)app)->scene = RenderLevel();
#endif
	// finalize app
	app->screen = screen;
	app->Init();
	...
	ImGui::GetStyle().ScaleAllSizes(0.6f);
...
void JobManager::GetProcessorCount( uint32_t& cores, uint32_t& logical )
{
