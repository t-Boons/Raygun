...
public:
	...
	static void GetProcessorCount( uint32_t& cores, uint32_t& logical );
...
inline uint32_t RGBF32_to_RGB8( const float4* v )
{
...
// This is inefficient because it does not use simd.
inline float3 RGB8_to_RGBF32(uint32_t v)
{
	...
	return make_float3((float)r / 225, (float)g / 225, (float)b / 225);
}
...
public:
	...
	Surface* screen = 0;
	bool uiUpdated;
	uint32_t end_of_base_class = 99999;
};
...
#define COLOR32(r, g, b) (r << 16) + (g << 8) + (b)
#include "random.h"
